...
from typhoon.core.runtime import BrokerInterface, TaskInterface, SequentialBroker
...
def make_airflow_tasks(task: TaskInterface, source_airflow_task: PythonOperator = None, custom_source_id: str = None):
    ...
    if isinstance(task.broker, SequentialBroker):
        for destination in task.destinations:
            ...
            for other_destination in destination.destinations:
                make_airflow_tasks(other_destination, airflow_task, custom_source_id=other_destination.task_id)
    else:
        airflow_task = make_airflow_task(source_id, task)
        if source_airflow_task is not None:
            source_airflow_task >> airflow_task
        for destination in task.destinations:
            make_airflow_tasks(destination, airflow_task)
