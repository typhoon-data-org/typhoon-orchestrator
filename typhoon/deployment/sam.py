...
def deploy_sam_template(dags: Sequence[Union[dict, DAG, DAGDefinitionV2]], remote: Optional[str] = None):
    sam_template = generate_sam_template(
        dags=[dag.dict() if isinstance(dag, DAG) or isinstance(dag, DAGDefinitionV2) else dag for dag in dags],
